...
template<typename T>
void debug_print_helper(const string& var_name, const T& var) {
    cout << var_name << " = " << var << ' ';
}

template<typename T, typename... Types>
void debug_print_helper(const string& vars_name, const T& var, const Types&... rest) {
    auto comma_pos = vars_name.find(',');
    cout << vars_name.substr(0, comma_pos) << " = " << var;
    auto next_comma = vars_name.find(',', comma_pos + 1);
    debug_print_helper(vars_name.substr(comma_pos + 1, vars_name.size()), rest...);
}

template<typename... Types>
void debug_print(const string& vars_name, const Types&... var) {
    debug_print_helper(vars_name, var...);
}