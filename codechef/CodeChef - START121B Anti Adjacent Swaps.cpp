...
// URL: https://www.codechef.com/START121B/problems/ANADSW
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
// 
    ...
    // }
// 
    // // Check if there is at least one non-adjacent pair to swap
    // for (int i = 0; i < n - 2; ++i) {
        // if (A[i] > A[i + 2]) {
            // return true;
        // }
...
bool elements(vector<int> &array,int n) {
  int swaps = 0;
  for (int i = 1; i < n; i++) {
...
void solve(){
    ...
    if(n<=2){
        ...
        if(max(a[2],a[0])< a[1]|| a[1]<min(a[2],a[0])){
