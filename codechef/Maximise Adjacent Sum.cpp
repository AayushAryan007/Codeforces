...
// URL: https://www.codechef.com/START123B/problems/MAXADJSUM
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
long long adjacentSum(const vector<int>& A) {
    long long sum = 0;
    for (int i = 0; i < A.size()-1; ++i) {
        sum += (static_cast<long long>(A[i])+ A[i+1]) ;
    }
    return sum;
}
...
int main() {
    ...
    while (t--) {
        ...
        // Swap the smallest 2 elements to the first and last positions
     
        swap(A[1], A[n - 1]);

        long long ans = adjacentSum(A);
        cout << ans << endl;
    }
