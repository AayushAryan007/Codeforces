...
bool solve()
{
    ...
    vector<int>v(n), p(n+1,-1), q(n+1,-1);
    vector<ll> ansp(n,0), ansq(n,0);
    set<ll> setp, setq;
    ...
    for(ll i=0;i<n;i++){
    	if(p[v[i]]==i){
    		auto itr=setq.upper_bound(v[i]);
    		
    		if(itr==setq.begin()) return false;
    		...
    		auto itr=setp.upper_bound(v[i]);
    		
    		if(itr==setp.begin()) return false;
