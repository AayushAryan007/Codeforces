...
void solve() {
    ...
    if (good.size() == 0) {
        ...
        ll maxwind = 1;
        ll l = good[0], r = good[0]; // Initialize l and r with first element of good
        ll i = 0, j = 0;
        ll curlen = 1; // Initialize curlen to 1, as there's at least one element

        while (j + 1 < good.size()) { // Ensure j + 1 does not exceed vector size
            if (good[j + 1] - good[j] == 1) {
                j++;
                curlen++;
            } else {
                if (curlen > maxwind) {
                ...
                i = ++j; // Move both i and j to the next element
                curlen = 1; // Reset curlen to 1
            }
        }

        if (curlen > maxwind) { // Check if the last sequence is the longest
            maxwind = curlen;
            l = good[i];
            r = good[j];
        }
