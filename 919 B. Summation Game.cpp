...
void solve(){
	...
	int right=n-1;
	 ...
	 if(left>0) ans=pref[left-1]+pref[left-1]-pref[right];
	 else ans= -1*pref[right];
...
//      	 
     	 // for(int i=n-x)
     // }
	 	right=i-1;left=right+1-x;
	 	
	 	if(left>0) ans=max(pref[left-1]+pref[left-1]-pref[right],ans);
	 	else ans=max(-1*pref[right],ans);
	 }
	 cout<<ans<<endl;
