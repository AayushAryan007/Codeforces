...
void solve()
{
     ...
     // fr(i,0,n){
     	// cout<<v[i]<<" ";
     // }cout<<endl;
   //  lower bound of element finds that element or finds an elementbigger than that
   
   // upper bound always finds the bigger that the asked element
   //these return location , iterator or address
   // int *ptr=lower_bound(v,v+n,5);
   // cout<<(*ptr)<<endl;
   auto it = upper_bound(v.begin(), v.end(), 5);
if (it != v.end()) {
    int index = it - v.begin(); // Get the index of the found element
    cout << index << endl;
} else {
