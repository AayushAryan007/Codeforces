...
// {
   ...
   // sort(all(v));
    // for (int i = 0; i < n; i++)
    // {
        // size_t j = 0; // Change the type of j to size_t
        // while (j <= vv.size() + 1)
        // {
            // if (vv[j].size() == 0)
            // {
...
void solve() {
    ...
    for (i = 0; i < n;) {
        ...
        cp.push_back(make_pair(v[j], i - j));
    }
    ...
    for (ll i = 1; i < n; i++) {
        if (cp[i].first + 1 == cp[i - 1].first) {
            ans += max(cp[i].second - cp[i - 1].second, 0LL);
        } else {
            ans += cp[i].second;
        }
    }
