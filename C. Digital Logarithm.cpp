...
void solve()
{
     ...
     //remove repeatation from both vectors
     for(auto x: fa){
     	ll num=x.first;
     	ll minm=min(fa[num],fb[num]);
     	fa[num]-=minm;
     	fb[num]-=minm;
     }
     
     //all >9 digit number to single digit
     ...
     // again remove repeatation of single digit elements
      for(auto x: fa){
     	ll num=x.first;
     	ll minm=min(fa[num],fb[num]);
     	fa[num]-=minm;
     ...
     //make remaining element 1 digit and count;
     for(auto x:fa){
     	if(x.first!=1){
     		ans+=x.second;
     	}
