...
//     
    // for (int i = 0; i < n; i++) {
        // if (s[i] >= start && s[i] <= end) { // Compare characters alphabetically
...
// if(ans[0]>ans[m-1]){
	// reverse(all(ans));
// }
 // char first = tolower(s[0]); // Convert first character to lowercase
    // char last = tolower(s[s.size() - 1]); // Convert last character to lowercase
// 
    // int solo = abs(last - 'a' - (first - 'a')); 
// // int solo= abs(end - start) - 'a';
//    
  // cout<<solo<<" "<<m<<endl;
   // for(int i=0;i<m;i++){
   	// cout<<ans[i]<<" ";
