...
void solve()
{
    ...
    stack<pair<char, int>> upp;
    stack<pair<char, int>> low;

    for (int i = 0; i < s.length(); i++) {
        if (isupper(s[i])) {
            if (s[i] != 'B') {
                upp.push(make_pair(s[i], i));
            } else if (!upp.empty()) {
                upp.pop();
            }
        } else {
            if (s[i] != 'b') {
                low.push(make_pair(s[i], i));
            } else if (!low.empty()) {
                low.pop();
            }
        }
    ...
    while (!upp.empty() || !low.empty()) {
        if (!upp.empty() && !low.empty()) {
            if (upp.top().second > low.top().second) {
                ans += upp.top().first;
                upp.pop();
            } else {
                ans += low.top().first;
