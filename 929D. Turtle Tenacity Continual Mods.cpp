...
// 
// 
// bool can_rearrange(vector<int>& arr) {
    // int n = arr.size();
// 
...
// 
    // // Check if min_value occurs only once or there exists another value v such that v % min_value != 0
    // int min_occurrences = count[min_value];
    // int other_value_count = 0;
// 
    // for (auto it = count.begin(); it != count.end(); ++it) {
        // int num = it->first;
        // int occurrences = it->second;
// 
        // if (num != min_value && num % min_value != 0) {
            // other_value_count += occurrences;
        // }
    // }
// 
    // return min_occurrences == 1 || other_value_count > 0;
// }

bool can_rearrange(vector<int>& v){
	int n=v.size();
	int mini=INT_MAX;
