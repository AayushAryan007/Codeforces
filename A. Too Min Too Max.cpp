...
//     
    ...
    // int maxi = INT_MIN;
//     
    // // Two pointers approach to find elements with maximum difference
    // while (i < j) {
        // maxi = max(maxi, abs(v[i] - v[j]) + abs(v[j - 1] - v[i + 1]) + abs(v[i + 1] - v[j - 1]));
        // i++;
        // j--;
    // }
// 
    // // Nested loops for the remaining elements
    // for (int k = 1; k <= n; k++) {
//     
            // for (int l = 1; l <= n; l++) {
//                
                    // if (i != j && j != k && k != l && l != i && l != j && k != i) {
                        // maxi = max(maxi, abs(v[i] - v[j]) + abs(v[k-1] - v[j]) + abs(v[k-1] - v[l-1]) + abs(v[i] - v[l-1]));
                    // }
//                 
            // }
//         
