...
bool solve()
{
     ...
     vector<vector<ll>> minm(n,vector<ll>(m,inf)),maxm(n,vector<ll>(m,minf));
     ...
     minm[0][0]=grid[0][0], maxm[0][0]=grid[0][0];
     ...
     for(ll i=1;i<n;i++){
     	for(ll j=1;j<m;j++){
     		minm[i][j]=min(minm[i][j-1]+grid[i][j],minm[i-1][j]+grid[i][j]);
     		maxm[i][j]=max(maxm[i][j-1]+grid[i][j],maxm[i-1][j]+grid[i][j]);
     	}
     
     }	return minm[n-1][m-1]<=0 and maxm[n-1][m-1]>=0;
}
