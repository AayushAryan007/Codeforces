...
void min_cost(int t, vector<vector<int>>& cases) {
    // Iterate through each test case
    for (int i = 0; i < t; ++i) {
        int n = cases[i][0];    int b = cases[i][2];
        int a = cases[i][1];
        
        
    
        // Calculate the cost of buying n yogurts individually and in pairs
        int cost_individual = n * a;
        
        
        int cost_pairs = (n / 2) * b + (n % 2) * a < a * n ? (n / 2) * b + (n % 2) * a : a * n;
        // Output the minimum cost
        cout << min(cost_individual, cost_pairs) << endl;
    }
}
...
signed main() {
    ...
    vector<vector<int>> cases(t, vector<int>(3)); // Vector to store test cases
    ...
    // Call the function to calculate and output the minimum cost for each test case
    min_cost(t, cases);
