...
void getnumbers(int n)
{
    ...
    for(int i=2;i*i<=n;i++) //till sqrt of number
    {
    	while(n%i==0){
    		divs.pb(i);
    		n/=i;
    ...
    //if(n is still !=1 then its a prime number)
    if(n!=1){
    	divs.pb(n);
    }
    ...
    //use loop to check if the list of divisors has three distinct one for our answer
    ...
    //check if we have found 3 distinct
