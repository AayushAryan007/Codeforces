...
// fr(i, 0, n) {
    ...
    // cin >> x;
// 
    // // Include pairs where x < i+1
    // if (x < i + 1) {
        // vp.push_back({x, i + 1});
    // }
// }
// 
// // Sort based on the first element of the pair (x values)
// sort(all(vp));
// // 
// // debug(vp);
// // 
...
// for (ll i = 0; i < n - 1; i++) {
    // ll j = i + 1;
 // // ll flag=0;
    // // Count pairs where vp[i].second >= vp[j].second
    // // while (j < n && vp[i].second >= vp[j].first) {
// //        
        // // // if(vp[i].second>=vp[j].s)
         // // j++;
    // // }
    // count+=upper_bound(all(v),vp[i].second)-v.begin();
// 
    // // count += n - j;
// }
// 
...
//      
     ...
     // // fr(i,0,n){
     	// // fr(j,i+1,n){
     		// // if(v[i]<i+1 and i+1<v[j] and v[j]<j+1) count++;
     	// // }
     // // }
     // // cout<<count<<endl;
// }
...
void solve(){
	...
	// debug(v)
	for(auto i:v){
		count+=lower_bound(all(v),a[i-1])-v.begin();
		}
		cout<<count<<endl;
}
int main()  
